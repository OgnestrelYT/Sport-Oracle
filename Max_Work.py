...
class MyWidget(QMainWindow):
    ...
    def saveTable(self):
        ...
        dmd = self.date.split('.')
        if set(self.b) == 2:
            x = self.cur.execute("DELETE FROM " + f"'{str(min(self.b)) + '-' + str(max(self.b))}'").fetchall()
            for i in range(1, a + 1):
                try:
                    dmd = self.Main_Table.item(i - 1, 0).text().split('.')
                    x = '''INSERT INTO''' + f"'{str(min(self.b)) + '-' + str(max(self.b))}'" + '''(id, Year, Mounth, Day, Score, Result) VALUES(?,?,?,?,?,?);'''
                    self.result = self.cur.execute(x, (i, dmd[0], dmd[1], dmd[2], self.Main_Table.item(i - 1, 1).text(),
                                                       self.Main_Table.item(i - 1, 2).text())).fetchall()
                except:
    ...
    def loadTable(self):
        ...
        if self.file.count(".") == 0:
            if self.file != "":
                ...
                for i in range(a):
                    self.Main_Table.removeRow(0)
                x = self.cur.execute("SELECT name FROM sqlite_master WHERE type='table';")
                self.result = x.fetchall()
                self.b = list(map(int,self.result[0][0].split('-')))
                self.Team_Name_Text.setText("Таблица: " + self.file)
                self.Error_Text.setText("")
                self.Save_Table_Button.setEnabled(True)  # теперь можно сохранять таблицу
                self.Sessions_Table_Button.setEnabled(True)
                if len(set(self.b)) == 2:
                    self.result = self.cur.execute(''' SELECT *  FROM ''' + f"'{str(min(self.b)) + '-' + str(max(self.b))}'").fetchall()
    ...
    def sessions(self):
        ...
        self.result = self.cur.execute(''' SELECT *  FROM Session''').fetchall()
        self.result.sort(key=lambda x: (x[1], x[2], x[3]))
